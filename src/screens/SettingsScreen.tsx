...
const SettingsScreen = ({
  ...
  return (
    <div className="px-4 pt-4 pb-6 overflow-y-auto h-full relative">
      ...
      <Section title="YOUR STORES">
        ...
        >
          <div className="text-left">
            <p className="text-sm font-medium text-foreground">Manage Stores</p>
      ...
      {/* Store Names */}
      <Section title="STORE NAMES">
        {selectedStoreIds.length > 0 ? (
          selectedStoreIds.map(storeId => {
            const store = stores.find(s => s.storeId === storeId);
            if (!store) return null;
            return (
              <div key={storeId} className="flex items-center gap-3 py-3 border-b border-border last:border-0">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground">{storeOverrides[storeId] || store.displayName}</p>
                  <p className="text-[11px] font-mono text-muted-foreground">{storeId}</p>
                </div>
          ...
          <p className="text-xs text-muted-foreground py-3">No stores selected. Manage stores from the section above.</p>
        )}
      </Section>

      {/* App Preferences */}
      <Section title="APP PREFERENCES">
        <ToggleRow icon={<Moon size={16} />} label="Dark Mode" value={darkMode} onChange={setDarkMode} />
        ...
        <div className="flex items-center gap-3 py-3">
          <span className="text-muted-foreground"><Key size={16} /></span>
          <span className="text-sm text-foreground flex-1">Default Date Range</span>
        ...
        {editingStore && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <h3 className="text-sm font-semibold text-foreground mb-3">Edit Store Name</h3>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="sticky top-0 z-10 bg-background px-4 pt-4 pb-4 border-b border-border flex items-center gap-3 mb-4">
        ...
        <h1 className="text-lg font-bold text-foreground">Select Stores</h1>
      </div>
